...
'use client';
import { DollarSign, TrendingDownIcon, TrendingUpIcon, User2Icon, UserCheck2Icon } from "lucide-react"
...
export function SectionCards() {
    const { data: UserData, isLoading: UserLoading, isError: UserError, mutate: UserMutate } = useFetch('/api/dashboard/sectioncard');
  ...
  return (
    <>
    <Card className="rounded-xl bg-muted/50" onClick={() => router.push('/admin/capel-ret')}>
    <CardHeader className="relative">
      <CardDescription className=''>Total Calon Pelanggan</CardDescription>
    ...
    <CardFooter className="flex-col items-start gap-1 text-sm">
      <div className="line-clamp-1 flex gap-2 font-medium">
        Belum di verifikasi <User2Icon className="size-4" />
      </div>
      <div className="text-muted-foreground">
        Untuk dilakukan survey lapangan 
      </div>
    </CardFooter>
  </Card>
  <Card className="rounded-xl bg-muted/50" onClick={() => router.push('/admin/pelanggan')}>
    <CardHeader className="relative">
    ...
    <CardFooter className="flex-col items-start gap-1 text-sm">
      <div className="line-clamp-1 flex gap-2 font-medium">
        Tertagih Retribusi pada bulan ini <UserCheck2Icon className="size-4" />
      </div>
      <div className="text-muted-foreground">
